...
function type() {
    ...
    if (index < currentText.length) {
        ...
        setTimeout(type, 300); // Delay between letters
    } else {
...
function deleteText() {
    ...
    if (index > 0) {
        index--;
        updateText(); // Update text and position the cursor
        setTimeout(deleteText, 300); // Delay between each letter deletion
    } else {
        // Switch to the next word
        currentTextIndex = (currentTextIndex + 1) % texts.length;
        setTimeout(type, 50); // Delay before starting the next word
    }
}
